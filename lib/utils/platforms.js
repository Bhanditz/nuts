...
function detectPlatform(platform) {
    ...
    if (_.contains(name, 'deb') || hasSuffix(name, '.deb')) prefix = platforms.DEB;

    if (_.contains(name, 'rpm') || hasSuffix(name, '.rpm')) prefix = platforms.RPM;

    if (_.contains(name, 'linux')
        || _.contains(name, 'ubuntu')
        || hasSuffix(name, '.tgz')
        || hasSuffix(name, '.tar.gz')
        || hasSuffix(name, '.tar.xz')) prefix = platforms.LINUX;
    ...
    if (_.contains(name, '32')
        ...
        || _.contains(name, '386')) suffix = '32';
    if (_.contains(name, '64') || _.contains(name, 'x64') || _.contains(name, 'amd64') || _.contains(name, 'x86_64')) suffix = '64';

    suffix = suffix || ((prefix !== platforms.WINDOWS) ? '64' : '32');
