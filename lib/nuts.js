...
function Nuts(opts) {
    ...
    this.router.get('/whoami', this.onWhoami);
    this.router.get('/badge', this.onBadge);
...
function fromUserAgent (useragent) {
  var platform
  if (useragent.isMac) platform = platforms.OSX;
  if (useragent.isWindows) platform = platforms.WINDOWS;
  if (useragent.isLinux) platform = platforms.LINUX;
  if (useragent.isLinux64) platform = platforms.LINUX_64;
  if (/Ubuntu; Linux/.test(useragent.source)) {
    if (useragent.isLinux64) {
      platform = platforms.DEB_64
    } else {
      platform = platforms.DEB_32
    }
  }
  if (/Fedora; Linux/.test(useragent.source)) {
    if (useragent.isLinux64) {
      platform = platforms.RPM_64
    } else {
      platform = platforms.RPM_32
    }
  }
  return platform
}

Nuts.prototype.onWhoami = function(req, res, next) {
    var that = this; 
    var platform = fromUserAgent(req.useragent);
    res.format({
...
Nuts.prototype.onBadge = function(req, res, next) {
    var that = this;
    ...
    return that.versions.resolve({
      ...
      var versionString = version.tag.replace(/-.*$/, '');
      var badgeUrl = 'https://img.shields.io/badge/latest%20version-' + versionString + '-ff8080.svg';
...
Nuts.prototype.onDownload = function(req, res, next) {
    ...
    if (!filename) {
        ...
        if (!platform) {
          platform = fromUserAgent(req.useragent)
        }
